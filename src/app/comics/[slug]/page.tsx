...
export default function ComicDetailPage({ params }: ComicPageProps) {
  ...
  const [isLoading, setIsLoading] = useState(!comic);
  ...
  useEffect(() => {
    ...
    if (local) {
      const rawEpisodes = Array.isArray(local.episodes) ? local.episodes : [];
      const sorted = {
        ...local,
        episodes: [...rawEpisodes].sort((a, b) => (a.episodeNumber || 0) - (b.episodeNumber || 0)),
      };
      setComic(sorted);
      setIsLoading(false);
      setIsBookmarked(dataStore.isBookmarked(sorted.id));
      setIsLiked(dataStore.isLiked(sorted.id));
      setLikesCount(sorted.views || sorted.likesCount || 1420);
    ...
    // 2. Fetch from robust Server API route
    fetch(`/api/comics/${encodeURIComponent(resolvedParams.slug)}`)
      .then((res) => res.json())
      .then((data) => {
        if (!isCurrent) return;
        if (data.success && data.comic) {
          const cloudComic = data.comic;
          const rawEpisodes = Array.isArray(cloudComic.episodes) ? cloudComic.episodes : [];
          const sorted = {
            ...cloudComic,
            episodes: [...rawEpisodes].sort((a, b) => (a.episodeNumber || 0) - (b.episodeNumber || 0)),
          };
          ...
          // Fallback to direct client dbService
          dbService
            .getComicBySlug(resolvedParams.slug)
            .then((cloud) => {
              if (!isCurrent) return;
              if (cloud) {
                const rawEpisodes = Array.isArray(cloud.episodes) ? cloud.episodes : [];
                const sorted = {
                  ...cloud,
                  episodes: [...rawEpisodes].sort((a, b) => (a.episodeNumber || 0) - (b.episodeNumber || 0)),
                };
  ...
  }, [resolvedParams.slug, user?.id]);

  const episodes = Array.isArray(comic?.episodes) ? comic.episodes : [];
  const hasEpisodes = episodes.length > 0;
  const activeEpisode = hasEpisodes ? (episodes[activeEpisodeIdx] || episodes[0]) : null;
  const pages = Array.isArray(activeEpisode?.imageUrls) ? activeEpisode.imageUrls : [];

  // Auto-record reading progress safely
  useEffect(() => {
    if (!comic || !hasEpisodes || !activeEpisode) return;
    dataStore.saveReadingProgress({
      ...
      creatorName: comic.creator?.name || "Original Creator",
      chapterNumber: activeEpisode.episodeNumber || 1,
      episodeTitle: activeEpisode.title || `Episode ${activeEpisodeIdx + 1}`,
      progressPercentage: Math.round(((activeEpisodeIdx + 1) / (episodes.length || 1)) * 100),
      totalUnits: episodes.length,
      lastReadAt: new Date().toISOString(),
    });
  }, [comic, activeEpisodeIdx, hasEpisodes, activeEpisode]);
  ...
  const isCreator = Boolean(
    user && (user.id === comic.creatorId || user.username === creator.username)
  );
  ...
  return (
    <div className={`min-h-screen pb-24 space-y-6 ${isFullscreen ? "bg-black text-white" : ""}`}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 space-y-4">
        {/* If no episodes published yet: Display Webtoon-Style Cinematic Teaser Card */}
        {!hasEpisodes ? (
          <div className="py-14 px-6 sm:px-12 rounded-3xl bg-gradient-to-b from-zinc-900/95 via-zinc-900/80 to-zinc-950 border border-indigo-500/20 text-center space-y-6 max-w-3xl mx-auto shadow-2xl relative overflow-hidden backdrop-blur-xl animate-in fade-in zoom-in-95">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 text-xs font-black tracking-wider uppercase">
              <span className="w-2 h-2 rounded-full bg-indigo-400 animate-ping" />
              <span>Series In Production • Coming Soon</span>
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs sm:text-sm text-zinc-400 max-w-xl mx-auto leading-relaxed">
                The author has officially registered this story universe. Episode 1 panels are currently in preparation. Subscribe to receive an instant notification when Episode 1 drops!
              </p>
            </div>

            {/* Action Bar for Creator vs Reader */}
            <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
              {isCreator ? (
                <>
                  <Link
                    href={`/creator/upload?mode=ADD_CHAPTER&seriesId=${comic.id}&type=COMIC`}
                    className="px-6 py-3 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs shadow-lg shadow-emerald-600/25 transition flex items-center gap-2 transform hover:scale-105 active:scale-95"
                  >
                    <Upload className="w-4 h-4" />
                    <span>Upload Episode 1 Now</span>
                  </Link>
                  <Link
                    href="/creator"
                    className="px-5 py-3 rounded-2xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-bold text-xs transition"
                  >
                    Creator Studio
                  </Link>
                </>
                ...
                <>
                  <button
                    onClick={handleBookmarkToggle}
                    className={`px-6 py-3 rounded-2xl text-xs font-black transition flex items-center gap-2 shadow-lg transform hover:scale-105 active:scale-95 ${
                      isBookmarked
                  ...
                  >
                    ...
                    <span>{isBookmarked ? "Subscribed to Series" : "🔔 Subscribe & Notify Me"}</span>
                  </button>
                  ...
                  >
                    ...
                    <span>{likesCount} Favorites</span>
                  </button>
                  ...
                  <button
                    ...
                    className="px-5 py-3 rounded-2xl bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs font-bold transition flex items-center gap-1.5"
                  >
          ...
          <>
            {/* Floating Reader Toolbar (Only when episodes exist) */}
            <div className="sticky top-16 z-30 flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl bg-zinc-900/95 border border-zinc-800 backdrop-blur-md text-xs text-zinc-300 shadow-2xl">
              <div className="flex items-center gap-2">
                <span className="font-bold text-white text-xs sm:text-sm">
                  {activeEpisode ? `Ep. ${activeEpisode.episodeNumber}: ${activeEpisode.title}` : "No Episodes Published"}
                </span>
          ...
          {hasEpisodes && (
            <div className="flex items-center justify-between">
              <button
                ...
                onClick={() => {
                  setActiveEpisodeIdx((prev) => Math.max(0, prev - 1));
              ...
              <span className="text-xs font-bold">
                Episode {activeEpisodeIdx + 1} of {episodes.length}
              </span>

              <button
                disabled={activeEpisodeIdx >= episodes.length - 1}
                onClick={() => {
                  setActiveEpisodeIdx((prev) => Math.min(episodes.length - 1, prev + 1));
                  setActivePageIndex(0);
                }}
              ...
              <div className="space-y-3">
                ...
                {episodes.length === 0 ? (
                  <p className="text-xs text-zinc-500 py-6 text-center">
                    No episodes uploaded yet for this series.
                  </p>
                ) : (
